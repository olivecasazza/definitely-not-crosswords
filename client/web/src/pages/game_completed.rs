...
pub fn GameCompleted(id: String) -> Element {
    ...
    rsx! {
        style { {COMPLETED_CSS} }
        div {
            style: "flex: 1; width: 100%; max-width: 56rem; margin: 0 auto; padding: 2rem 1rem; overflow-y: auto;",

            match &*data_res.read_unchecked() {
                None => rsx! {
                ...
                Some(Ok(Some(data))) => {
                    ...
                    let total_questions = data.game.questions.as_ref().map(|q| q.len()).unwrap_or(0);
                    let total_guesses: i64 = rankings.iter().map(|r| r.correct_guesses + r.incorrect_guesses).sum();
                    let total_correct: i64 = rankings.iter().map(|r| r.correct_guesses).sum();
                            ...
                            div { style: "display: grid; grid-template-columns: 1fr; gap: 2rem;",
                                ...
                                div { style: "display: flex; flex-direction: column; gap: 1rem;",
                                    div { style: "display: flex; align-items: center; justify-content: space-between; font-family: monospace; padding: 0 .25rem;",
                                        ...
                                        span { class: "muted", style: "font-size: .625rem; text-transform: uppercase;", "{rankings.len()} Players" }
                                    }
